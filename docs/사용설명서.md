...
└─ .claude/
    ...
    │   ├─ chief-of-staff.md · product-manager.md · researcher.md
    │   ├─ designer.md · architect.md · implementer.md · devops.md
    │   ├─ reviewer.md · security.md · legal.md · data-analyst.md · scribe.md
    │   ├─ auditor.md        # 감사팀 (시스템 자체 점검)
    ├─ workflows/             # ⚙️ 협업 프로세스
    │   ├─ go.js             # ⭐ 단일 통합 명령 (인원배정→…→문서)
    │   ├─ build-feature.js · staffed-build.js · standup.js · audit.js
    └─ skills/
...
```
> 🖥️ **이건 계속 떠 있어야 하는 "서버"입니다.** Claude Code 세션과 **별도의 터미널**에서
> 띄우고, 보는 동안 **그대로 켜 두세요**(끄면 화면도 멈춤). 끌 때는 그 터미널에서 **Ctrl+C**.
> 한 번 실행하면 브라우저가 1.5초마다 자동 갱신하므로 새로고침할 필요는 없습니다.

브라우저로 http://localhost:4317/ 를 열고 상단 **🎬 Office view** 클릭. 기본은 **실제 작업이
있을 때만** 움직입니다(실제 작업이 돌면 자동으로 살아납니다).

**3단계 — 일 시키기**
메인 세션에 자연어로 지시하면 됩니다. 예:
- *"architect 에이전트로 로그인 기능 설계해줘"* → 한 명에게 위임
- **`/go /health 엔드포인트 추가`** → 인원 배정 + 조사·설계·구현·검토·문서를 한 번에 (이 명령 하나면 됨)
...
| **product-manager** | 🧭 | "무엇을·왜 / 요구사항·범위·우선순위" (맨 앞 단계) | Read·Grep·Glob·Write | opus |
| **designer** | 🎨 | "UX/UI·사용자 흐름·화면·사용성" | Read·Grep·Glob·Write | opus |
| **devops** | 🛠️ | "배포·CI/CD·인프라·신뢰성" | Read·Edit·Write·Grep·Glob·Bash | opus |
| **data-analyst** | 📊 | "지표·분석·실험" | Read·Grep·Glob·Bash·Write | sonnet |
| **security** | 🔒 | "위협 모델·취약점·보안 검토" | Read·Grep·Glob·Bash | opus |
| **legal** | ⚖️ | "법률·라이선스·IP·개인정보·규제 검토 — 의존성 라이선스 호환·컴플라이언스" | Read·Grep·Glob·WebSearch·WebFetch·Write·Bash | opus |

> ⚖️ **legal은 변호사가 아닙니다** — 컴플라이언스 리서치와 기술적 체크를 수행하는 역할이지
> 법적 자문이 아닙니다. 애매하거나 고위험이거나 특허가 걸린 사안은 사람 변호사에게
> 에스컬레이션합니다.
...
| 워크플로우 | 흐름 | 용도 |
|---|---|---|
| **`go`** ⭐ | **인원배정 → 조사 → 설계 → 구현 → 검토 → 문서** | **이것 하나면 끝.** `/go <목표>` |
| standup | 정찰(researcher) → 브리핑(chief-of-staff) | 현황 보고 |
| audit | 정찰(researcher) → 감사(auditor) | 시스템 자체 점검 + 개선 결재 |
| (내부) build-feature / staffed-build | `go`의 구성요소 | 보통 직접 쓸 필요 없음 |

> ⭐ **`/go`가 단일 통합 명령**입니다 — 예전엔 "회사 활용(build-feature)"과 "인원 배정
> (staffed-build)"이 따로였지만, 이제 `/go <목표>` 하나가 **인원 배정 + 전체 실행**을 한 번에 합니다.
> 전체 파이프라인은 조사 → 설계 → 구현 → 검토 → **보안 → 법무(legal)** → 데이터 분석 → 문서
> 순으로 진행됩니다(자세한 순서는 [ARCHITECTURE.md](ARCHITECTURE.md) 참고).
...
| 배정 | 기준 |
|---|---|
| **0명** | 그 역할이 필요 없음 (예: 문서 불필요 → scribe 0, 외부/법적 노출 없음 → legal 0) |
| **1명** | 단순·단일 작업 (예: legal — 서드파티 의존성/라이선스, 사용자 데이터/PII, IP, 규제 도메인이 있을 때) |
...
사무실은 **정사각형 화면 안의 4×4 그리드**로, 부서를 **워크플로우 순서대로(실제 회사의
부서 인접 배치 연구를 반영)** 묶어 둡니다. 복도는 없고, 각 방은 **상단 이름 띠(plaque)** 로
구분됩니다.

```
┌─────────────────────── 🏢 사무실 (정사각형 4×4 그리드) ───────────────────────┐
│ 👑 대표(You)    🎩 총괄         📋 관리팀        🕵️ 감사팀      │  ← 리더십
│ 🧭 기획팀       🔍 조사팀       🎨 디자인팀      📊 데이터팀    │  ← Product
│ 📐 설계팀       🔨 개발팀       🛠️ 데브옵스                     │  ← Engineering
│ 🛡️ 검토팀       🔒 보안팀       ✍️ 문서팀                       │  ← 품질·문서
└────────────────────────────────────────────────────────────────────────────┘
   👑 대표는 좌상단 전용 코너 오피스 · ✈️ 종이비행기 = 에이전트 간 메시지(워크플로우 순서로 흐름)
```

**화면 읽는 법**
- 🧑‍💻 **에이전트(사람)** — 작업중이면 **선명해지며 액센트색 윤곽선(글로우)** 이 생기고,
  일 안 하면 **흐릿(회색조)**, 완료는 **초록 ✓**. 윤곽선 유무로 한눈에 구분됩니다.
- 🏷️ **작업 캡션은 작업중일 때만** 사람 아래에 표시(예: `자료 조사`) — 완료/대기면 사라집니다.
- 🗂️/📦 **서류함·완료 창고와 화면은 "현재 세션"의 작업만** 보여줍니다(세션 단위로 관리 — 전역
  누적 아님). 다른 세션의 작업이 화면을 어지럽히지 않습니다.
- 👑 **대표(You)** — 좌상단 **전용 코너 오피스**. **각 부서(또는 카드)를 클릭하면** 그
  에이전트가 이번 세션에 **한 작업 목록**이 팝업으로 뜹니다. 👑 대표를 클릭하면 **팀 전체 작업**을 한 번에.
  - 각 작업은 **`'<주제>'에 대한 자료 조사`처럼 구체적 주제**로 표시됩니다(훅이 캡처한 실제 작업
    설명에서 추출). **항목을 클릭하면 그 아래로 전체 작업 내용이 펼쳐집니다**(다시 클릭하면 접힘).
- 🌐 **언어 토글**(상단 `EN`/`한국어` 버튼) — 한국어/영어 전환(저장됨). 한국어일 때 **부서 이름은
  볼드**로 표시되고, 작업은 직관적인 역할 기반 라벨(조사팀 → "자료 조사" / Research → "researching")
  에 위 주제가 결합됩니다.
- ✈️ **종이비행기** — 에이전트끼리 주고받는 **메시지 1건**. 라벨에 내용(`findings`,
  `design spec`, `PR diff` …)이 적혀 있고, **여러 개가 동시에** 날아갑니다.
  - **보내는 사람** 📤 / **받는 사람** 📥(받을 때 고개 듦)
  - 비행기 색 = **보낸 부서 색** (출처 추적)
  - 출발 전 보내는 에이전트 위에서 **1초 대기** 후 **일정한 속도로 순항**, 도착하면 사라집니다.
- 🟡 working · 🟢 done · ⚪ idle (하단 범례)

> **모션 = 실제 작업** — 화면은 **실데이터로만** 움직입니다(별도 데모/배경 연출 없음). 실제
> 에이전트가 일할 때만 윤곽선이 살아나고, 실제 핸드오프가 일어날 때만 종이비행기가 날며,
> **비행기 라벨에는 그 에이전트의 실제 작업 텍스트**가 붙습니다. 실제 워크플로우가 돌면 사무실이
...
| **전부 한 번에 (추천)** | **`/go <목표>`** — 인원 배정 + 조사·설계·구현·검토·문서까지 한 명령으로 |
| 현황 보고 / 시스템 점검 | `/standup` · `/audit` |
...
| 캐릭터가 다 졸고 있음 | 실제 작업이 없다는 뜻(정상). `/go <목표>`를 실행하면 살아남 |
