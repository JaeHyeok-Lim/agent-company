...
**1단계 — 이 폴더에서 세션 열기 (중요)**
프로젝트 전용 에이전트·hook은 **`agent-company` 폴더에서 Claude Code를 열 때만** 적용됩니다.

**2단계 — 대시보드 실행**
```bash
npm run dashboard       # → http://localhost:4317/
```
브라우저로 열고 상단 **🎬 Office view** 클릭 → 사무실이 움직입니다(기본 자동).
...
- **⏸ Pause motion** 버튼으로 모든 모션 정지/재생.

> 동작 원리: Claude Code **hooks**가 서브에이전트 시작/종료를 `.claude/state/agents.json`에
> 기록 → 대시보드가 폴링해서 그립니다. 실제 워크플로우가 돌면 그 상태가, 아니면 기본
> "바쁜 사무실" 앰비언트 애니메이션이 표시됩니다.
...
**전역 재사용** — 검증된 역할 정의(`.claude/agents/<role>.md`)를
`C:\Users\jaehyeok\.claude\agents\` 로 복사하면 **모든 프로젝트**에서 그 역할을 쓸 수 있습니다.
...
| 종이비행기가 안 날아감 | 실제 작업 없을 땐 앰비언트로 자동 비행. ⏸이 Pause 상태인지 확인 |
| 캐릭터가 다 졸고 있음 | 실제 state가 idle. 워크플로우를 돌리거나 앰비언트(기본)면 정상 |
| hook이 상태를 안 남김 | hook은 **프로젝트 폴더에서 연 세션**에서만 발화 (Windows는 `$CLAUDE_PROJECT_DIR` 치환) |
| 역할(subagent)이 안 보임 | 프로젝트 전용 agent는 그 폴더 세션에서만 등록됨 |
