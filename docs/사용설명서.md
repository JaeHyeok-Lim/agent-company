...
**화면 읽는 법**
- 🧑‍💻 **에이전트(사람)** — 책상에 앉아 일합니다. **타이핑 모션 = 작업중**, **고개 꾸벅 + 💤 = 대기**.
- 🏷️ **각 사람 아래 캡션** — 지금 무슨 작업을 하는지 (예: `scan auth`, `module A`).
...
> **모션 = 실제 작업 (기본)** — 기본 화면은 **실데이터로만** 움직입니다. 실제 에이전트가
> 일할 때만 타이핑하고, 실제 핸드오프가 일어날 때만 종이비행기가 날며, **비행기 라벨에는
> 그 에이전트의 실제 작업 텍스트**가 붙습니다. 아무 일도 없으면 다들 졸고(idle) 비행기도
> 없습니다. — 즉 **움직임이 곧 실제 활동의 신호**입니다.
>
> **🎬 Demo motion** 버튼을 켜면(기본 OFF), 실제 작업이 없을 때 "바쁜 사무실" 쇼케이스
> 애니메이션(가짜 비행기 + 복도 보행자)이 돌아갑니다. 발표/미리보기용입니다.

> 동작 원리: Claude Code **hooks**가 서브에이전트 시작/종료와 **작업 내용**을
> `.claude/state/agents.json`에 기록 → 대시보드가 1.5초마다 폴링해서 그립니다.
> ⚠️ **hook은 `agent-company` 폴더에서 연 세션에서만 발화**합니다(전역 발화는 아래 참고).
...
| (선택) hook | `~/.claude/settings.json` | 전역 발화 — 모든 프로젝트 상태 기록 시 |
...
| 종이비행기가 안 날아감 | 기본은 **실제 핸드오프 때만** 비행. 미리보기는 **🎬 Demo motion** ON |
| 캐릭터가 다 졸고 있음 | 실제 작업이 없다는 뜻(정상). 워크플로우를 돌리거나 Demo motion ON |
| 다른 프로젝트 작업이 화면에 안 뜸 | hook이 `agent-company` 폴더 세션에서만 발화 — 전역 표시는 hook 전역화 필요(요청 시 설정) |
