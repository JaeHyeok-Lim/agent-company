...
- 👑 **You (CEO)** — 화면 상단의 당신 캐릭터. **각 캐릭터(또는 카드)를 클릭하면** 그 에이전트가
  이번 세션에 **한 작업 목록**이 팝업으로 뜹니다. 👑 You를 클릭하면 **팀 전체 작업**을 한 번에.
- 🌐 **언어 토글**(상단 `EN`/`한국어` 버튼) — 한국어/영어 전환(저장됨). 작업은 기술 용어 대신
  **직관적인 역할 기반 라벨**(예: 조사팀 → "자료 조사" / Research → "researching")로 표시됩니다.
